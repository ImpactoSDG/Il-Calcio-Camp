  ...
  <div class="container-fluid p-4 bg-white min-vh-100 position-relative animate-fade-in">
    ...
    <div v-if="ventasAbiertasAntiguas.length > 0" class="alert-ventas-viejas mb-4 animate-fade-in shadow-sm">
      <div class="d-flex align-items-center gap-3">
        ...
        <div>
          <h6 class="fw-bold mb-1 text-danger">Ventas olvidadas de días anteriores</h6>
          <p class="mb-0 small text-muted">
            Hay {{ ventasAbiertasAntiguas.length }} venta(s) que quedaron abiertas desde antes de hoy. 
            Por favor, revisalas y cierralas o elimínalas para mantener la caja al día.
          </p>
        </div>
        ...
        <div class="modal-olvidadas">
          ...
          <div class="modal-olvidadas__body">
            <p class="text-muted small mb-3">
              Estas ventas quedaron abiertas antes de hoy. Hac clic en una para abrirla.
            </p>
            <div class="table-responsive">
...
/** Construye la URL pblica de un smbolo dado su nombre de archivo (ej: "estrella.png") */
...
const handleQuickTeamAssign = async ({ id_cliente, id_equipo }) => {
  try {
    ...
    if (!String(id_cliente).startsWith('temp-')) {
      ...
      // Se persistir cuando se guarde la venta completa
    ...
    toast.showToast({ message: 'No se pudo persistir la asociacin del equipo.', type: 'warning' });
  }
};
...
const ventasFiltradas = computed(() => {
  let items = (ventas.value || []).map(v => {
    ...
    // Si la venta tiene detalle de artculos (por el getVentaById o similar), usamos la primera imagen
    const url_imagen_principal = v.articulos_venta?.[0]?.url_imagen || v.url_imagen_principal || null;
    
    // Asegurarse de que total_venta sea un nmero vlido antes de formatear
...
const openVentaModal = async (item = null, forceCierre = false) => {
  if (item) {
    // MODO EDICIN / AGREGAR ARTCULOS
    isEditing.value = true;
    loadingDetalle.value = true;
    try {
      // Cargamos los artculos actuales de la venta para que aparezcan en el modal
...
const saveArticuloVenta = async () => {
  ...
  try {
    ...
    toast.showToast({ message: 'Artculo agregado.', type: 'success' });
...
const confirmDeleteArticuloVenta = async () => {
  ...
  try {
    ...
    toast.showToast({ message: 'Artculo quitado.', type: 'success' });
...
onMounted(async () => {
  ...
  // Cargar certificados QZ Tray para esta mquina y luego pre-conectar
  await initQzSecurity();
  ensureQzConnected().catch(() => console.warn('QZ Tray no disponible al inicio.'));
  
  // Asegurar que la impresora est sincronizada: preferencia de mquina tiene prioridad sobre el global
  try {
    const machinePreference = getMachinePreferredPrinter();
    if (machinePreference) {
      // Re-aplicar la preferencia especfica de esta mquina (por si el localStorage fue limpiado)
      saveMachinePreferredPrinter(machinePreference);
    } else {
      // Sin preferencia especfica: usar el predeterminado global
...
/* Imagen del smbolo anti-falsificacin */
.simbolo-badge {
  width: 18px;
  height: 18px;
  object-fit: contain;
...
/* -- Estilos de Impresin (Ticket 80mm) -------------------------- */
@media print {
  /* Ocultar todo el contenedor de la aplicacin y modales */
  ...
  /* Asegurar que el ticket sea lo nico visible */
  .ticket-impresion {
