  ...
  <div class="container-fluid p-4 bg-white min-vh-100 position-relative animate-fade-in">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 pb-3 border-bottom">
      <div class="d-flex align-items-center">
        ...
        <h1 class="h5 fw-bold mb-0 text-secondary">ARBITROS</h1>
      </div>
    ...
    <div class="mb-3">
      <FuzzySearch v-model="searchQuery" placeholder="Buscar por nombre, apellido, DNI, teléfono o email..." />
    </div>
          ...
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title">
                ...
                {{ isEditing ? 'Editar Arbitro' : 'Nuevo Arbitro' }}
              </h5>
              ...
              <div class="modal-body">
                <div class="row g-3">
                  <div class="col-md-6">
                    ...
                    <input v-model.trim="form.nombre" type="text" class="form-control" placeholder="Ej: Nestor" required />
                  </div>
                  <div class="col-md-6">
                    <label class="form-label">Apellido</label>
                    <input v-model.trim="form.apellido" type="text" class="form-control" placeholder="Ej: Pitana" required />
                  </div>
                  <div class="col-md-4">
                  ...
                  <div class="col-md-4">
                    ...
                    <input v-model.trim="form.telefono" type="text" class="form-control" placeholder="Ej: 3511234567" />
                  </div>
                  <div class="col-md-4">
                    <label class="form-label">Email</label>
                    <input v-model.trim="form.email" type="email" class="form-control" placeholder="Ej: arbitro@mail.com" />
                  </div>
                  <div class="col-12">
                    <div class="form-check form-switch ps-4">
                      <input v-model="form.activo" class="form-check-input" type="checkbox" role="switch" id="chkActivoArbitro" />
                      <label class="form-check-label fw-semibold ms-2" for="chkActivoArbitro">
    ...
    <ConfirmModal
      ...
      message="¿Estás seguro de eliminar este arbitro? Si luego queda vinculado a eventos, la base puede restringir la operación."
...
const fetchData = async () => {
  loading.value = true;
  try {
    const data = await datosMaestrosService.getArbitros();
    arbitros.value = data.map(item => ({
      ...item,
      activo: Boolean(Number(item.activo)),
    }));
  } catch {
    toast.showToast({ message: 'Error al cargar los arbitros.', type: 'danger' });
  } finally {
    loading.value = false;
  }
};
...
const save = async () => {
  ...
  if (form.value.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.value.email.trim())) {
    toast.showToast({ message: 'El email no tiene un formato valido.', type: 'warning' });
    return;
  }
  ...
  if (isEditing.value && JSON.stringify(buildPayload()) === JSON.stringify({
    ...
    telefono: originalForm.value.telefono || null,
    email: originalForm.value.email || null,
  ...
  try {
    if (isEditing.value) {
      await datosMaestrosService.actualizarArbitro(buildPayload());
      toast.showToast({ message: 'Arbitro actualizado correctamente.', type: 'success' });
    } else {
      await datosMaestrosService.crearArbitro(buildPayload());
      toast.showToast({ message: 'Arbitro creado correctamente.', type: 'success' });
    }
    showFormModal.value = false;
    fetchData();
  } catch {
    toast.showToast({ message: 'Error al guardar el arbitro.', type: 'danger' });
  } finally {
...
const confirmDelete = async () => {
  ...
  try {
    await datosMaestrosService.eliminarArbitro(idToDelete.value);
    toast.showToast({ message: 'Arbitro eliminado correctamente.', type: 'success' });
    showDeleteModal.value = false;
    fetchData();
  } catch {
    toast.showToast({ message: 'Error al eliminar el arbitro.', type: 'danger' });
  } finally {
